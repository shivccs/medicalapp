...
class add_allergy extends MX_Controller {
	...
	public function add(){
	   ...
	   $count = $this->doctor_profile_model->count_row('allergy_name',$allergy,'allergies');
	

	if($count>=1)
	{
		 $this->session->set_flashdata('msg', 'allergy already exsist!');
				  redirect(base_url().'template/add_allergy/');
		...
		if (isset($_POST['flag'])) {
			...
			$result = $this->doctor_profile_model->insert_data('allergies',$u_data);
