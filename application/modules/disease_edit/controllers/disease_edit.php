...
class disease_edit extends MX_Controller {
	...
	{	
		...
		$this->load->model('doctor_profile_model');
		 ...
		 $rec=$this->doctor_profile_model->get_specific_data('cronic_disease_id',$id,'cronic_disease');
	...
	public function edit(){
		...
		if (isset($_POST['flag'])) {
			...
			$result = $this->doctor_profile_model->edit_data('cronic_disease_id',$aid,$u_data,'cronic_disease');
           
			if ($result) {
			$this->session->set_flashdata('msg', 'cronic disease successfully Updated');
			  redirect(base_url().'template/disease_edit/'.$aid);
			}else{
