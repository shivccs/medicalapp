<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Add_disease_model extends CI_model {
	

	function check_disease($dis){
		$this->db->select('*');
		$this->db->where('disease_name', $dis);
		$q = $this->db->get('disease');
		if ($q->num_rows()>0) {
			return true;
		}else{
			return false;
		}
	}//end of function check_role

	function register($cdata){
		$this->db->trans_begin();

		$r1 = $this->db->insert('disease', $cdata);

		if ($r1==true) {
			$this->db->trans_commit();
			return true;

		}else{
			 $this->db->trans_rollback();
			 return false;
		}
	}//end of function 


	function get_disease(){
		$this->db->select('d.*, CONCAT(u.first_name, " ", u.last_name) as added_by');
		$this->db->from('disease d');
		$this->db->join('users u', 'u.user_id=d.created_by');
		$this->db->order_by('d.disease_name', 'asc');
		$q = $this->db->get();
		if ($q->num_rows()>0) {
			return $q->result_array();
		}else{
			return false;
		}
	}//end of function check_mobile

}//end of class