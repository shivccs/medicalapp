...
class symptoms_edit extends MX_Controller {
	...
	{	
		...
		$this->load->model('doctor_profile_model');
		 ...
		 $rec=$this->doctor_profile_model->get_specific_data('symptoms_id',$id,'symptoms');
	...
	public function edit(){
		...
		if (isset($_POST['flag'])) {
			...
			$result = $this->doctor_profile_model->edit_data('symptoms_id',$aid,$u_data,'symptoms');
            echo $result;
			if ($result) {
			$this->session->set_flashdata('msg', 'Symptoms successfully Updated');
			  redirect(base_url().'template/symptoms_edit/'.$aid);
			}else{
