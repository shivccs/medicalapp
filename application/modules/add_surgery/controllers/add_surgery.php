...
class add_surgery extends MX_Controller {
	...
	public function add(){
	   ...
	   $count = $this->doctor_profile_model->count_row('surgery_name',$allergy,'surgeries');
	

	if($count>=1)
	{
		$this->session->set_flashdata('msg', 'surgery already exsist!');
				  redirect(base_url().'template/add_surgery/');
		...
		if (isset($_POST['flag'])) {
			...
			$result = $this->doctor_profile_model->insert_data('surgeries',$u_data);
           
			if ($result) {
			$this->session->set_flashdata('msg', 'surgeries successfully Updated');
			 redirect(base_url().'template/add_surgery/');
			}else{
